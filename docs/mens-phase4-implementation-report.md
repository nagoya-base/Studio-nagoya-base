...
1. **男性ペア・男性グループが同時に写った写真が0点**：Phase3 3章が明記する制約。「男性同士」を画像で直接示す実写真がなく、現在のギャラリーは単独モデルの実写真のみで構成されている（9章のOGP画像はイラストのため対象外）。
2. **学校机・黒板を使った男性作例が0点**：設備としては備品リストに存在するが、撮影例の写真がない。
3. **ユニフォーム以外の衣装（制服・スポーツウェア単体・レザー等）を主題にした実写真が0点**：現行の人物写真はすべて同一モデル・野球ユニフォームまたはヒーローコスチュームである。
4. **実ブラウザでのスマホ表示・アクセシビリティ実機確認が未実施**：320〜768px幅での目視確認、FAQ・ギャラリーモーダルの実機動作確認、キーボード操作・コントラストの確認を推奨。
5. **「利用の流れ」の共通include化**：Phase 2の16章が示す将来課題として未着手（`mens/index.html` に個別記述のまま）。
