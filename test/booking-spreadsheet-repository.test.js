 ...
 * 'paymentRecoveryReason'の末尾17列だけを1回のsetValuesで更新する。updateBooking
 * CancellationStateAtomic/updateBookingPriceBaselineAtomicと同じ設計で、'paymentStatus'
 * （30列目。この17列とHEADERS_上で連続していない）や間に挟まる決済リンク送信・料金・
 * 日程変更精算関連の既存列は一切書き込まない。
 */
test('updateBookingPaymentStateAtomic: paymentAttemptId〜paymentRecoveryReasonの17列だけを1回の書き込みで更新し、paymentStatusと中間の既存列は変化しない', function () {
  var sandbox = loadRepos();
  sandbox.SpreadsheetRepository.appendBooking(sampleRecord({
  ...
  assert.strictEqual(lastCall.numCols, 17, 'paymentAttemptId〜paymentRecoveryReasonの17列だけを1回で書く（PR #354レビュー対応・3回目でpaymentAttemptResolvedAtを追加）');
  assert.strictEqual(lastCall.numRows, 1);
});
