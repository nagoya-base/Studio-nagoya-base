...
test('buildFeeConfirmation_: feeReadyかつ差額なしならexpectedFeeMasterVersionのみのconfirmationを返す', function () {
  var sandbox = loadClientSandbox();
  var section = fakeSection({});
  var confirmation = sandbox.buildFeeConfirmation_(section, {
    feeReady: true, feeMasterVersion: 3, requiresManualRefundDecision: false
  });
  assert.deepStrictEqual(Object.keys(confirmation).sort(), ['expectedFeeMasterVersion']);
  assert.strictEqual(confirmation.expectedFeeMasterVersion, 3);
});
...
test('buildFeeConfirmation_: 返金要判断で理由が空ならnullを返す（送信をブロックする）', function () {
  ...
  var confirmation = sandbox.buildFeeConfirmation_(section, {
    feeReady: true, feeMasterVersion: 1, requiresManualRefundDecision: true, unrefundedPaidAmount: 8000
  });
...
test('buildFeeConfirmation_: 返金要判断で妥当な入力ならmanualRefundDecisionを含めて返す', function () {
  ...
  var section = fakeSection({ '#reschedule-refund-amount': amountField, '#reschedule-refund-note': noteField });
  var confirmation = sandbox.buildFeeConfirmation_(section, {
    feeReady: true, feeMasterVersion: 2, requiresManualRefundDecision: true, unrefundedPaidAmount: 8000
  });
  assert.strictEqual(confirmation.expectedFeeMasterVersion, 2);
