...
- **データソース・アルゴリズム**（`package.json`のゼロ依存方針を維持するため、npm
  ...
  - ハッピーマンデー対象（成人の日・海の日・敬老の日・スポーツの日）は年ごとの日付を
    列挙せず、「第n月曜」を毎年計算で求める。
  - 春分の日・秋分の日は、国立天文台の実測値と1980〜2099年の範囲で一致することが
    広く確認されている天文計算の近似式で算出する（年ごとの手書きテーブルではない）。
...
- **対応年の範囲**: 2020〜2099年（`JapaneseHolidays.MIN_SUPPORTED_YEAR`/
  ...
  近似式の精度が確認できている範囲。予約は将来日付のみが対象のため、現行の運用では
  問題にならない。2090年代に入ったら近似式の精度を再確認し、上限の延伸要否を判断する
  こと。国会が祝日法を改正した場合（2020/2021のような一時的特例、恒久的な祝日の
  追加・変更のいずれも）は、`JapaneseHolidays.gs`内の該当箇所を追記・修正すること。
- **タイムゾーン**: 施設タイムゾーン（`Asia/Tokyo`）基準に正規化済みの`YYYY-MM-DD`
  文字列を前提とする（`BookingPricing.gs`の既存方針をそのまま踏襲。ブラウザ/GAS実行
  環境のローカルタイムゾーンには依存しない）。
- **判定不能時はfail-closed**: 対応年の範囲外など、`JapaneseHolidays.classify`が
  祝日区分を確定できない場合、`resolveDayType_`は黙って`WEEKDAY`にフォールバックせず、
  `computeBookingPrice`が`valid: false`を返す。`estimatePrice`（Code.gs）・
  `createBooking`（BookingRepository.gs）はいずれも既存の`priceResult.valid`チェックが
  そのままこのエラーを見積り・予約作成エラーとして扱う（過少請求を避ける。新しい
  分岐を追加する必要はなかった）。
